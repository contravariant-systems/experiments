...
print(
    f"  - Cyclic coordinates: {sys.cyclic_coordinates if sys.cyclic_coordinates else 'none'}"
)
print()
...
axes[0].axhline(
    threshold, color="r", linestyle="--", alpha=0.5, label=f"Threshold = {threshold}"
)
axes[0].axvline(
    diverge_time,
    color="g",
    linestyle="--",
    alpha=0.5,
    label=f"Divergence at t={diverge_time:.1f}s",
)
...
axes[1].plot(
    x2_a[::10], y2_a[::10], "b-", linewidth=0.3, alpha=0.7, label="Trajectory A"
)
axes[1].plot(
    x2_b[::10], y2_b[::10], "r-", linewidth=0.3, alpha=0.7, label="Trajectory B"
)
axes[1].set_xlabel("x")
axes[1].set_ylabel("y")
...
early_phase = (
    separation[: int(diverge_idx * 0.8)] if diverge_idx > 100 else separation[:1000]
)
